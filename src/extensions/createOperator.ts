 ...
 * and must return an `EnumeratorFactory<TResult>`.
 *
 ...
 * createOperator({
 *     name: 'everyNth',
 *     factory(source: Enumerable<unknown>, n: number) {
 *         return {
 *             getEnumerator() {
 *                 return everyNthGenerator(source[Symbol.iterator](), n) as unknown as Enumerator<unknown>;
 *             }
 *         };
 *     },
